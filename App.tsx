...
const initial: Form = { department: 'Spoonitöötlus', process: '', title: '', description: '', photo: null, name: 'Slepko Aleksander', category: '', priority: priorities[2] };
...
export default function OMSApp({ screen }: { screen: 'form' | 'tickets' }) {
  ...
  return <SafeAreaView style={[styles.safe, Platform.OS === 'android' && { paddingTop: Math.max(0, (StatusBar.currentHeight ?? 0) - insets.top) }]} edges={['top', 'left', 'right']}>
...
const styles = StyleSheet.create({
  ...
  overlay: { flex: 1, justifyContent: 'flex-end', backgroundColor: '#081C30A8' }, sheet: { maxHeight: '85%', backgroundColor: 'white', borderTopLeftRadius: 20, borderTopRightRadius: 20, padding: 20, paddingBottom: 28 }, sheetTitle: { color: colors.ink, fontSize: 20, fontWeight: '800', marginBottom: 12 }, optionList: { flexShrink: 1 }, option: { minHeight: 54, flexDirection: 'row', alignItems: 'center', borderBottomWidth: 1, borderColor: '#EBEFF2' }, optionText: { flex: 1, color: colors.ink, fontSize: 16 }, optionCheck: { color: colors.blue, fontSize: 20, fontWeight: '700' }, cancel: { alignItems: 'center', marginTop: 15, padding: 12 }, cancelText: { color: colors.blue, fontSize: 15, fontWeight: '700' },
